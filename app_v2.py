...
def export_to_cidoc_ttl(nodes_map, edges_list):
    ...
    def safe_uri_frag(name):
        if not name: return "unknown"
        # 특수문자 및 공백 제거 (한글, 한자, 영문, 숫자 유지)
        cleaned = re.sub(r"[^0-9A-Za-z\u4e00-\u9fff\uac00-\ud7a3\u1100-\u11ff\u3130-\u318f]+", "_", str(name))
    ...
    for nid, info in nodes_map.items():
        raw_id = info.get("raw_id", nid)
        frag = safe_uri_frag(raw_id)
        color = info.get("color", "#999999")
        rdf_type = type_map.get(color, "crm:E1_CRM_Entity")
        
        # 표시용 레이블에서 이모지 및 줄바꿈 제거
        clean_label = re.sub(r"[\u2600-\u27BF\U0001f300-\U0001f64f\n]", "", info.get("label", "")).strip()
        
        lines.append(f"ex:{frag} a {rdf_type} ;")
        lines.append(f'    rdfs:label "{clean_label}" .')
        lines.append("")
    ...
    for edge in edges_list:
        ...
        src_info = nodes_map.get(src_id)
        tgt_info = nodes_map.get(tgt_id)
        
        if src_info and tgt_info:
            src_frag = safe_uri_frag(src_info.get("raw_id", src_id))
            tgt_frag = safe_uri_frag(tgt_info.get("raw_id", tgt_id))
            
            # CIDOC-CRM 속성 표준 매핑 명문화
            crm_pred = label
            if label == "P14_carried_out_by":
                crm_pred = "crm:P14_carried_out_by"
            elif label == "P7_took_place_at":
                crm_pred = "crm:P7_took_place_at"
            elif not label.startswith("crm:") and not label.startswith("ex:"):
                crm_pred = f"ex:{label}"
                
            lines.append(f"ex:{src_frag} {crm_pred} ex:{tgt_frag} .")
...
# ==========================================
st.set_page_config(layout="wide", page_title="3.1 운동 도슨트")
st.title("🇰🇷 3.1 운동 역사 도슨트")
...
with c1:
    ...
    if nodes_map:
        ttl_data = export_to_cidoc_ttl(nodes_map, edges_list)
        st.download_button(
            label="📥 현재 검색 결과 TTL 다운로드",
            data=ttl_data,
            file_name=f"history_docent_{hashlib.sha1(search_term.encode('utf-8')).hexdigest()[:8]}.ttl",
            mime="text/turtle",
...
with c2:
    ...
    if evidences or pg_texts:
        ...
        st.markdown("**RAG 설정**: DeepSeek 모델을 우선 사용하며, 없으면 간단 요약(fallback)을 제공합니다.")
