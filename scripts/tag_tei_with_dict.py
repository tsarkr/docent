...
BIBLIOGRAPHY_TITLE_EXACT = {
    ...
    '警務總監部', '京城地方法院', '京城覆審法院', '高等法院', '平壤地方法院', '朝鮮總督府裁判所書記', '朝鮮總督府判事', '豫審掛職務代理朝鮮總督府判事',
    '豫審掛朝鮮總督府判事', '豫審掛代理朝鮮總督府判事', '豫審掛朝鮮總督府裁判所書記', '豫審掛職務代理朝鮮總督府裁判所書記',
    '朝鮮總督府裁判所書記', '朝鮮總督府判事', '裁判長', '判事', '司法警察官事務取扱', '警務總監部', '警部', '書記', '巡査'
}
...
class LocalDocentEngine:
    ...
    def _clean_and_pre_tag_hanja(self, text):
        ...
        def replace_hanja(match):
            ...
            # [가드 3] 인명 첫 글자 두음법칙 정밀 반영
            if len(reading) >= 2:
                first_char = reading[0]
                
                # 3-1. 'ㄹ/ㄴ'이 'ㅣ, ㅑ, ㅕ, ㅛ, ㅠ' 앞에 올 때 'ㅇ'으로 교정 (리계창 -> 이계창)
                ...
                # 3-2. 'ㄹ'이 'ㅏ, ㅓ, ㅗ, ㅜ, ㅡ, ㅐ, ㅔ' 앞에 올 때 'ㄴ'으로 교정 (로윤길 -> 노윤길)
    ...
    def _is_valid_person_name(self, name):
        ...
        if len(name) > 2 and name.endswith(('군', '면', '읍', '리', '시', '구')):
            return False

        exact_noise_tokens = {
            '군', '현', '읍', '면', '리', '도', '시', '구', '촌', '포', '진', '령', '강', '천', '궁', '문', '묘', '역', '원', '해', '녀'
        }
        if name in exact_noise_tokens:
            return False
    ...
    def _normalize_person_candidate(self, name):
        ...
        if re.search(r'[\u4e00-\u9fff]', candidate) and not re.search(r'[가-힣]', candidate):
            ...
            if candidate.startswith('金') and reading.startswith('금'):
                reading = '김' + reading[1:]

            if candidate.startswith('丸山') and reading.startswith('환산'):
                reading = reading.replace('환산', '마루야마')
            elif candidate.startswith('楠') and reading.startswith('남'):
                reading = reading.replace('남', '구스노키')
    ...
    def _looks_like_person_name(self, name):
        candidate = str(name).strip()
        if not candidate or not re.fullmatch(r'[가-힣]{2,8}', candidate):
            return False
    ...
    def _collect_candidate_names_from_field(self, field_elem):
        candidates = []

        for term in field_elem.findall('.//term'):
            gloss = term.findtext('gloss')
            foreign = term.findtext('foreign')
            term_text = (gloss or foreign or ''.join(term.itertext())).strip()
            if term_text:
                candidates.extend(self._split_name_candidates(term_text))

        if not candidates:
            plain_text = ''.join(field_elem.itertext()).strip()
            candidates.extend(self._split_name_candidates(plain_text))
    ...
    def _extract_names_from_event_related_field(self, raw_text):
        ...
        for elem in root.iter():
            if elem.attrib.get('data-col') != '관련인물':
                continue
            ...
            for child in list(elem):
                if buffer and (';' in buffer or '；' in buffer):
                    ...
                    for segment in re.split(r'[;\n]+', buffer):
                        ...
                        for candidate in self._extract_last_token_names(prefix):
                            candidate = self._normalize_person_candidate(candidate)
                            if self._is_valid_person_name(candidate) and (
                                len(candidate) >= 3 or any(mark in segment for mark in ('(', '（', '['))
                            ):
                                buffer_candidates.append(candidate)

                    if buffer_candidates:
                        extracted.extend(buffer_candidates)
                ...
                if child_candidates:
                    extracted.extend(child_candidates)
    ...
    def _extract_names_from_top_title_field(self, raw_text):
        ...
        extracted = []
        for elem in root.iter():
            if elem.attrib.get('data-col') != '상위자료_제목':
                continue
    ...
    def _extract_names_from_source_info_fields(self, raw_text):
        ...
        particle_patterns = [
            r'\[[^\]]*\]',
            r'(?<=[\u4e00-\u9fff가-힣])(?=(?:은|는|이|가|도|와|과|의|를|을|에|에서|에게|께|등|및|로|으로|이며|인|란|라고|자|후|전|보다|까지|부터))',
        ]

        for elem in root.iter():
            data_col = elem.attrib.get('data-col')
            if data_col not in SOURCE_INFO_NAME_FIELDS:
                continue
                    ...
                    if re.fullmatch(r'[가-힣\s]+', segment):
                        for token in self._split_name_candidates(segment):
                            for candidate in self._split_compound_korean_name_tokens(token):
                                candidate = self._normalize_person_candidate(candidate)
                                if self._is_valid_person_name(candidate) and self._looks_like_person_name(candidate) and len(candidate) >= 3:
                                    extracted.append(candidate)
                        continue
    ...
    def _extract_names_from_structured_fields(self, raw_text, allowed_fields=None):
        ...
        for elem in root.iter():
            ...
            if allowed_fields is not None:
                if data_col not in allowed_fields:
                    continue
            elif data_col not in BIBLIOGRAPHY_PERSON_FIELDS:
                continue
            ...
            for term in elem.findall('.//term'):
                ...
                term_text = (gloss or foreign or ''.join(term.itertext())).strip()
                if term_text:
                    term_candidates.extend(self._extract_last_token_names(term_text))
    ...
    def run_full_pipeline(self, limit=0):
        ...
        # --- HITL export: extract entities to CSV, convert to XLSX, and pause for human review
        try:
            ...
            try:
                ...
                print(f'⚠️ XLSX 변환 실패: {e} — CSV 위치: {csv_out}')
            ...
            print(f'⚠️ HITL 전처리 단계에서 예외 발생: {e}')
